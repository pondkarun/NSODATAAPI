...
const Contens = () => {
    ...
    useLayoutEffect(() => {
        if (loadingpage.current) {
            ...
            API.get(`http://dookdik2021.ddns.net/services/v1/resource/by_id/${id}?option=full`).then(({ data }) => {
                setState(data);
            }).catch((error) => {
    ...
    const GetTabledataContens = () => {
        API.get(`http://dookdik2021.ddns.net/services/v1/data/content/${id}`).then(({ data }) => {
            console.log(data);
                ...
                <div className="card-container" style={{ marginTop: 50 }} >
                    <Tabs type="card">
                        <TabPane tab="Data Explorer" key="1">
                            <div>
                                <Table bordered scroll={{ x: true }} columns={column} dataSource={row} />
                            </div>
                        </TabPane>
